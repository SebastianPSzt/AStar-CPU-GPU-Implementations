...
void HeapifyUp(BinaryMinHeap *bmh, int index)
{
    ...
    // Parent <= Child Case
    if (parentNode->weight <= childNode->weight)
        return;
...
{
    if (!bmh || index < 0 || index >= bmh->size)
        return;

    int leftChildIndex = 2 * index + 1;
    int rightChildIndex = 2 * index + 2;

    if (leftChildIndex >= bmh->size)
        return;
    ...
    if (bmh->arr[index]->weight <= bmh->arr[minChildIndex]->weight)
        return;

    Node_BMH *temp = bmh->arr[index];
    bmh->arr[index] = bmh->arr[minChildIndex];
    bmh->arr[minChildIndex] = temp;
...
{
    BinaryMinHeap *bmh = malloc(sizeof(BinaryMinHeap));
