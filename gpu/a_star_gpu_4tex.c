...
int gridSize_x,
int gridSize_y,
cudaTextureObject_t gridData_d, 
...
int* bestMeetIndex_d)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        int cell = tex2D<int>(gridData_d, neighborIndex_x, neighborIndex_y);
        if (cell == 2) continue;
...
cudaTextureObject_t gridData_d, int* bucket_sizes_d, int* bucket_nodes_d, int* next_bucket_d, int* bestMeetCost_d, int* bestMeetIndex_d, int startBucket) {
    // ----- Variables -----
...
void Init_AStar(Grid_2D_Device* grid, int startIndex_x, int startIndex_y, int goalIndex_x, int goalIndex_y, int print) {
    ...
    cudaArray_t gridArray;
    cudaChannelFormatDesc desc = cudaCreateChannelDesc<int>();

    cudaMallocArray(&gridArray, &desc, gridSize_x, gridSize_y);
    cudaMemcpy2DToArray(gridArray, 0, 0, grid->data, gridSize_x * sizeof(int), gridSize_x * sizeof(int), 
    gridSize_y, cudaMemcpyHostToDevice);
    
    cudaResourceDesc resDesc = {};
    resDesc.resType = cudaResourceTypeArray;
    resDesc.res.array.array = gridArray;
    ...
    cudaTextureObject_t gridData_d;
    cudaCreateTextureObject(&gridData_d, &resDesc, &texDesc, NULL);
...
void Clean_AStar(int* gScore, int* forward_gScore_d, int* backward_gScore_d, cudaTextureObject_t gridData_d, 
int* bucket_sizes_d, int* bucket_nodes_d, int* next_bucket_d, int* bestMeetCost_d, int* bestMeetIndex_d) {
    ...
    cudaDestroyTextureObject(gridData_d);
